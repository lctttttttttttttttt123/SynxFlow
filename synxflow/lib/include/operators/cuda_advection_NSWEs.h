...
namespace GC{
  ...
  namespace fv{
    ...
    ///T2 route 甲-正 (c): h/hU advection AND fused passive-tracer hC advection in one stencil pass
    ///(reuses the live interface mass flux _h_flux; no separate rider, no cache array). h/hU output
    ...
    ///T6: trailing // [phos] args carry dissolved hPd + per-group particulate hPp_k on the SAME _h_flux
    ///(pointers; default nullptr / n_phos=0 = phosphorus off, kernel guards on hPd!=nullptr / n_phos).
    void cuAdvectionMSWEsCartesianWithSediment(cuFvMappedField<Scalar, on_cell>& gravity, cuFvMappedField<Scalar, on_cell>& h, cuFvMappedField<Scalar, on_cell>& z, cuFvMappedField<Vector, on_cell>& z_gradient, cuFvMappedField<Vector, on_cell>& hU, cuFvMappedField<Scalar, on_cell>& C, cuFvMappedField<Scalar, on_cell>& hC, cuFvMappedField<Scalar, on_cell>& h_advection, cuFvMappedField<Vector, on_cell>& hU_advection, cuFvMappedField<Scalar, on_cell>& hC_advection, Scalar** hCs_dev, Scalar** Csbound_dev, Scalar** hCs_adv_dev, int n_sed, Scalar* hPd = nullptr, Scalar* Pd_bound = nullptr, Scalar* hPd_adv = nullptr, Scalar** hPps_dev = nullptr, Scalar** Ppsbound_dev = nullptr, Scalar** hPps_adv_dev = nullptr, int n_phos = 0);
